...
2. Wait for the cluster to come up and then start the OpenShift router
   with two (_2_) replicas.
        ...
        $ vagrant ssh node-1  # (or master or node-2).
...
6. Ensure you can get to the hello openshift app from inside/outside the vm.
        ...
        $ #  node-{1,2} use IPs 10.245.2.{3,4} in the dev environment.
        for ip in 10.245.2.3 10.245.2.4; do
...
1. Copy the router HA settings example config and edit it as needed.
        ...
        $ cd /vagrant/hack/exp/router-failover
        $ cp conf/settings.example  settings.node-1
        $ cp conf/settings.example  settings.node-2
        $ #
...
2. For demo purposes, we are going to flip the PRIMARY and SLAVE groups
   on node-2 ... this allows both nodes to serve in an Active-Active
   fashion.

        $ #  Flip PRIMARY+SLAVE groups on node-2 ("Papoy?! Ah Papoy!!").
        ...
        $ diff conf/settings.example  settings.node-1
        $ diff conf/settings.example  settings.node-2
...
3. Optionally clear the config - just so that we have a completely clean
   ...
   reuse purposes.
        ...
        $ #  Run these commands on the nodes via vagrant ssh node-{1,2}
        $ #    sudo service keepalived stop
        $ #    sudo rm -f /etc/keepalived/keepalived.conf

        $ #  OkOk
        for m in node-1 node-2; do
...
4. Setup router HA with failover using the 2 config files we created.
        ...
        $ #  Run these commands on the nodes via vagrant ssh node-{1,2}
        $ #    cd /vagrant/hack/exp/router-failover
        $ #    sudo ./failover-setup.sh settings.node-{1,2}
...
nodes.
        ...
        $ vagrant ssh node-2
        #  Check that the VIPs from node-1 are taken over by this node.
        ip a ls dev enp0s8
...
Link quoted sound effects (ala "OkOk") to
        ...
        http://www.soundboard.com/sb/nodes
